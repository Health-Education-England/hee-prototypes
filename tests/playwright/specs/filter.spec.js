...
test('Filters: selection and clear', async ({ page }) => {
  ...
  // Ensure all parameters have been removed and the results hash is present.
  await expect(page).toHaveURL(new RegExp('.html\\?profession=medical_doctors&sub_profession=dental_nurse&filter=consultation&filter=guidance&results_updated=true$'));

  // Change selected "Sub profession" filter.
  await page.locator('select[name="sub_profession"]').selectOption({ label: 'Dentist' });

  // Ensure all parameters have been removed and the results hash is present.
  await expect(page).toHaveURL(new RegExp('.html\\?profession=dental_professionals&sub_profession=dentist&filter=consultation&filter=guidance&results_updated=true$'));

  // Click the "Profession" clear all link.
  await page.getByRole('link', {name: 'Clear'}).first().click();

  // Ensure all parameters have been removed and the results hash is present.
  await expect(page).toHaveURL(new RegExp('.html\\?profession=&sub_profession=&filter=consultation&filter=guidance&results_updated=true$'));
});
